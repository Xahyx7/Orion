...
if 'page' not in locals() or page == "Live Session" or (st.session_state.session_active and st.session_state.zen_mode):
    if not st.session_state.session_active:
        ...
        .flip-container {{ display: flex; gap: 15px; opacity: {"0.5" if st.session_state.session_paused else "1.0"}; }}
        .flip-box {{ background: #0f0f0f; border: 2px solid #222; border-radius: 12px; padding: 20px; min-width: 130px; text-align: center; position: relative; }}
        .flip-box::after {{ content: ''; position: absolute; top: 50%; left: 0; width: 100%; height: 2px; background: #000; }}
        .flip-num {{ font-family: 'Oswald', sans-serif; font-size: 100px; color: #00f2fe; margin: 0; }}
        .flip-label {{ font-family: sans-serif; color: #555; font-size: 12px; letter-spacing: 2px; }}
        </style>
...
# ==========================================
    ...
    if db.empty: st.warning("No data recorded.")
    else:
        tf = st.selectbox("Timeframe:", ["Today", "Yesterday", "This Week", "This Month", "This Year"])
        ...
        if fdb.empty: st.info(f"No data for {tf}.")
            ...
            col1.metric("Total Hours", f"{round(fdb['Duration'].sum()/60, 1)}")
